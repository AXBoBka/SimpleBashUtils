...
int main(int argc, char *argv[]) {
    ...
    if (!e_n && !f_n) {
        regex_arr = to_regex_compile(&current_position, argc, argv, regex_arr, &len, opt);
    }
    if (!err) {
...
regex_t *pars_options_e(int argc, char *argv[], int *e_n, const int *err, grep_opt opt) {
    int counter = 0;
...
regex_t *pars_options_f(int argc, char *argv[], int *f_n, int *err, grep_opt opt) {
    int i = 1;
    int counter = 0;
    ...
    while (i < argc && !(*err)) {
        if (!strcmp(argv[i], "-f") && i != argc - 1) {
            ...
            *(file_names + counter - 1) = malloc((strlen(argv[i]) + 1) * sizeof(char));
...
regex_t *regex_e_plus_regex_f(regex_t *regex_e, regex_t *regex_from_file, int e_cnt, int f_cnt) {
    if (regex_e == NULL) {
        regex_e = malloc(0);
    }
...
regex_t *to_regex_compile(int *current_position, int argc, char *argv[], regex_t *regex_arr,
                                        int *len, grep_opt opt) {
    regex_t *ret_reg = regex_arr;
    while (*current_position < argc - 1) {
        if (argv[*current_position][0] != '-') {
            char *tmp_str = malloc((strlen(argv[*current_position]) + 1) * sizeof(char));
...
void match_finder(grep_opt opt, regex_t *regex_arr, int len, int argc, char **argv, int current_position) {
    int mul_files = 0;
                        ...
                        if (!opt.c) {
                            ...
                            if (opt.o && !opt.v) {
                                ...
                                printf("%.*s\n", (int)(pmatch[0].rm_eo - pmatch[0].rm_so),
                                    &str[pmatch[0].rm_so]);
                                for (int k = 0; k < len; k++) {
                                    while (!regexec(regex_arr + k, str + appender, nmatch, pmatch, REG_NOTBOL)
...
regex_t *s21_get_regex_from_file(char **file_names, int len, int *f_n, int *error, grep_opt opt) {
    regex_t *regex_arr = NULL;
